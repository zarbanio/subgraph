...
export class SnapshotManager {
  ...
  private createOrUpdateMarketHourlySnapshot(): void {
    ...
    const id = this.market.id.concat(Bytes.fromI32(hours).toHexString());
  ...
  private createOrUpdateMarketDailySnapshot(): void {
    ...
    const id = this.market.id.concat(Bytes.fromI32(days).toHexString());
  ...
  createOrUpdateUsageHourlySnapshot(): void {
    const hours = this.event.block.timestamp.toI32() / SECONDS_PER_HOUR;
    const id = Bytes.fromI32(hours).toHexString();
  ...
  updateTransactionData(
    ...
    if (transactionType == TransactionType.DEPOSIT) {
      ...
      if (reserveFactor)
        this.financialSnapshot._cumulativeProtocolSideLiquidationRevenue =
          this.financialSnapshot._cumulativeProtocolSideLiquidationRevenue!.plus(amountUSD.times(reserveFactor))
  ...
  private getSnapshotRates(rates: string[], timeSuffix: string): string[] {
    ...
    for (let i = 0; i < rates.length; i++) {
      const rate = LmInterestRate.load(rates[i]);
      if (!rate) {
      ...
      const snapshotRate = new LmInterestRate(snapshotRateId);
  ...
  private getSnapshotRevenueDetail(
    ...
    const currDetails = LmRevenueDetail.load(currID);
    if (!currDetails) {
      log.error(
    ...
    const newDetails = new LmRevenueDetail(
      currDetails.id.concat(Bytes.fromI32(timeSuffix).toHexString())
    );
