...
export class TokenManager {
  ...
  updatePrice(
    ...
    newPriceTOMAN: BigDecimal = BIGDECIMAL_ZERO
  ): void {
    if (newPriceUSD != BIGDECIMAL_ZERO) {
      if (newPriceTOMAN == BIGDECIMAL_ZERO) {
        const dai = new TokenManager(DAI_ADDRESS, this.event);
        newPriceTOMAN = dai.getPriceTOMAN().times(newPriceUSD);
      }
      this.token.lastPriceBlockNumber = this.event.block.number;
      this.token.lastPriceTOMAN = newPriceTOMAN;
  ...
  getPriceTOMAN(): BigDecimal {
    if (this.token.lastPriceTOMAN) {
      return this.token.lastPriceTOMAN!;
    }
    return BIGDECIMAL_ZERO;
  }
  ...
  getOrCreateRewardToken(rewardTokenType: string): LmRewardToken {
    const rewardTokenID = rewardTokenType.concat("-").concat(this.token.id);
    let rewardToken = LmRewardToken.load(rewardTokenID.toLowerCase());
    if (!rewardToken) {
      rewardToken = new LmRewardToken(rewardTokenID);
